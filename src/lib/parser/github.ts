...
import {
  extractColorsFromRepo,
  extractImageAssetsFromRepo,
  extractTypographyFromRepo,
...
export async function parseGithubThemeFiles(repoInput: string): Promise<ParseResult> {
  ...
  const [colorResults, typographyResults, assets] = await Promise.all([
    extractColorsFromRepo(repoFullName),
    extractTypographyFromRepo(repoFullName),
    extractImageAssetsFromRepo(repoFullName),
  ]);
  ...
  const typographyRows = dedupeTypographyRows(
    typographyResults.flatMap((entry) => entry.typographyRows ?? []),
  );

  return { colors, typography, typographyRows, assets };
}
