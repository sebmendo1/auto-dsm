...
export { parseTailwindConfig } from "./tailwind-parser";

export function parseCode(content: string, fileType: "css" | "tailwind"): ParseResult {
  return fileType === "css" ? parseCSSVariables(content) : parseTailwindConfig(content);
}
...
export function autoDetectAndParse(content: string): ParseResult {
  ...
  return isTailwind ? parseTailwindConfig(content) : parseCSSVariables(content);
}
